...
@RequestScoped
public class MrpialBean implements Serializable {
    ...
    public void init() {
        ...
        while (it.hasNext()) {
            ...
            //TODO mrprecord
            Item mat = mb.getMat((long) pairs.getKey());
            List<MrpRecord> mrpr = mb.getMrpRecord(mf, mat);
            SuppliesMatToFac smtf = mb.getSmtf(mf, mat);
            ...
            int inv;
            if (mrpr.isEmpty()) {
                inv = mb.getInventoryIndiv(mb.getMat((long) pairs.getKey()),smtf.getFac());
            } else {
            ...
            mrpial.setMat(mb.getMat((long) pairs.getKey()));
    ...
    public void validateInput() throws IOException {
        ...
        for (int i = 0; i < mrpials.size(); i++) {
            SuppliesMatToFac smtf = mb.getSmtf(mf, mrpials.get(i).getMat());
            int leadtime = smtf.getLeadTime();
            if (leadtime == 1) {
    ...
    public void persistMrprecords() {
        ...
        for (MrpItemClass mrpial : mrpials) {
            System.out.println("onhadnafter:\n"
                    + "w4:" + mrpial.getWeek4().getOnh());
            List<MrpRecord> mrpr = mb.getMrpRecord(mf, mrpial.getMat());
            SuppliesMatToFac smtf = mb.getSmtf(mf, mrpial.getMat());
            int leadtime = smtf.getLeadTime();
